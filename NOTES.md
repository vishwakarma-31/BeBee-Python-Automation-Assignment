...
7. **Async rewrite** — Replace `ThreadPoolExecutor` + `requests` with
   `asyncio` + `httpx` for higher concurrency with lower memory overhead.
   Meaningful at 50+ concurrent workers.

8. **CAPTCHA solver hook** — Abstract a `CaptchaSolver` interface so a
   third-party service (2captcha, CapMonster) can be plugged in without
   changing core logic.

9. **Signup batch mode** — Extend `batch.py` to support CSV-driven bulk signup
   using a temp-mail service for email verification.

10. **Config file support** — Add YAML/TOML config so batch jobs don't need
   long CLI flags. Priority for production scheduling use cases.